...
#define ECRA_ALTURA 544   
#define ECRA_COMPRIMENTO 960
...
int main(int argc, char *args[])
{
	...
	while (emJogo)
	{
		...
		// Controlos - Setas da PS Vita
		sceCtrlPeekBufferPositive(0, &comando, 1);			
		if ((comando.buttons & SCE_CTRL_UP) && (jogador.dirVertical != 1)){
			jogador.dirVertical = -1;
			jogador.dirHorizontal = 0;
		}
		else if ((comando.buttons & SCE_CTRL_DOWN) && (jogador.dirVertical != -1)){
			jogador.dirVertical = 1;
			jogador.dirHorizontal = 0;
		}
		else if ((comando.buttons & SCE_CTRL_RIGHT) && (jogador.dirHorizontal != -1)){
			jogador.dirHorizontal = 1;
			jogador.dirVertical = 0;
		}
		else if ((comando.buttons & SCE_CTRL_LEFT) && (jogador.dirHorizontal != 1)){
			jogador.dirHorizontal = -1;
			jogador.dirVertical = 0;
		}
